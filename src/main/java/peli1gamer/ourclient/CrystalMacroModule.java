...
public final class CrystalMacroModule implements ToggleableModule {
    ...
    public void onClientTick(Minecraft mc) {
        ...
        if (!held) { useWasHeld = false; return; }
        useWasHeld = true;
    ...
    private void tryPlace(Minecraft mc, BlockHitResult hit) {
        ...
        int slot = findHotbar(mc.player, Items.END_CRYSTAL);
        if (slot < 0) return;
    ...
    private boolean lootNearby(Minecraft mc, BlockPos pos) {
        ...
        for (Entity entity : mc.level.getEntities((Entity) null, area)) {
            if (entity instanceof ItemEntity item && !item.getItem().isEmpty()) return true;
        }
        return false;
    }

    private int findHotbar(LocalPlayer player, net.minecraft.world.item.Item item) {
