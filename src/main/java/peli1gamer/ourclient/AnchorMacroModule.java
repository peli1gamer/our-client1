...
public final class AnchorMacroModule implements ToggleableModule {
    ...
    @Override public void onClientTick(Minecraft mc) {
        ...
        if (!enabled || player == null || mc.level == null || mc.gameMode == null || mc.screen != null || !player.isAlive()) {
            if (!isUseHeld(mc)) resetClocks();
            return;
        }
        ...
        if (!whileUse && (player.isUsingItem() || isGoodTool(player.getOffhandItem()))) return;
        ...
        if (charges(mc, pos) < (oneGlowstone ? 1 : 4)) charge(mc, hit);
        else detonate(mc, pos, hit);
    }

    private void trackOwnAnchorCandidate(Minecraft mc) {
        if (!mc.player.getMainHandItem().is(Items.RESPAWN_ANCHOR) || !(mc.hitResult instanceof BlockHitResult hit)) return;
    ...
    private void useHotbarItem(Minecraft mc, Item item, BlockHitResult hit) {
        ...
        mc.player.getInventory().setSelectedSlot(slot);
        try { rightClick(mc, hit); }
        finally { if (oldSlot != slot) mc.player.getInventory().setSelectedSlot(oldSlot); }
    }
    ...
    private boolean bodyNearby(Minecraft mc) {
        for (Entity e : mc.level.players()) {
            if (e != mc.player && e.distanceToSqr(mc.player) <= 36.0 && e instanceof LivingEntity l && l.isDeadOrDying()) return true;
        }
        return false;
    }
    ...
    private boolean valuablesNearby(Minecraft mc) {
        ...
        for (Entity e : mc.level.getEntities(null, area)) {
            ...
            } else if (s.getCount() > 32 && (s.is(Items.END_CRYSTAL) || s.is(Items.OBSIDIAN)
                    || s.is(Items.ENCHANTED_GOLDEN_APPLE) || s.is(Items.EXPERIENCE_BOTTLE))) return true;
        }
        return goodArmor >= 2;
    }
    ...
    private void resetState() { resetClocks(); knownAnchors.clear(); ownedAnchors.clear(); }
