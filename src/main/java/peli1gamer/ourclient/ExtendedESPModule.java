...
/** Native Arson extended ESP family. */
public final class ExtendedESPModule implements ToggleableModule, ConfigurableModule {
    public enum Mode { PLAYERS, MOBS, ITEMS, VILLAGERS, PILLAGERS, WANDERING_TRADERS, AMETHYST, BEEHIVES, DEEPSLATE, DRIPSTONE, KELP, VINES, LIGHT_BLOCKS }
    ...
    private static final List<ExtendedESPModule> INSTANCES = new ArrayList<>();
    ...
    public ExtendedESPModule(String id, Mode mode) {
        this.id = id;
        this.mode = mode;
    ...
    @Override public void setEnabled(boolean enabled) {
        ...
        if (!enabled) { matches.clear(); refreshTicks = 0; }
    }

    @Override public void onClientTick(Minecraft client) {
        if (!enabled || client.player == null || client.level == null) { matches.clear(); return; }
        if (isBlockMode()) {
            refreshTicks++;
    ...
    @Override public ModuleSettings settings() {
    ...
    private boolean isBlockMode() {
        return switch (mode) { case AMETHYST, BEEHIVES, DEEPSLATE, DRIPSTONE, KELP, VINES, LIGHT_BLOCKS -> true; default -> false; };
    }
    ...
    private static synchronized void refreshSharedBlockCache(Minecraft mc, boolean requested) {
        ...
        cachedCenter = center;
        BLOCK_CACHE.clear();
        for (Mode blockMode : new Mode[]{Mode.AMETHYST, Mode.BEEHIVES, Mode.DEEPSLATE, Mode.DRIPSTONE, Mode.KELP, Mode.VINES, Mode.LIGHT_BLOCKS}) BLOCK_CACHE.put(blockMode, new ArrayList<>());
        ...
        for (BlockPos pos : BlockPos.betweenClosed(min, max)) {
            ...
            for (Mode blockMode : BLOCK_CACHE.keySet()) {
                ...
                if (result.size() < BLOCK_LIMIT && matchesBlock(blockMode, block)) result.add(pos.immutable());
            }
        }
    }
    ...
    private static boolean matchesBlock(Mode mode, Block block) {
        return switch (mode) {
            case AMETHYST -> block == Blocks.AMETHYST_BLOCK || block == Blocks.BUDDING_AMETHYST || block == Blocks.AMETHYST_CLUSTER || block == Blocks.LARGE_AMETHYST_BUD || block == Blocks.MEDIUM_AMETHYST_BUD || block == Blocks.SMALL_AMETHYST_BUD;
            case BEEHIVES -> block == Blocks.BEE_NEST || block == Blocks.BEEHIVE;
            case DEEPSLATE -> block == Blocks.DEEPSLATE || block == Blocks.COBBLED_DEEPSLATE || block == Blocks.DEEPSLATE_BRICKS || block == Blocks.DEEPSLATE_TILES || block == Blocks.POLISHED_DEEPSLATE;
            ...
            case VINES -> block == Blocks.VINE || block == Blocks.WEEPING_VINES || block == Blocks.WEEPING_VINES_PLANT || block == Blocks.TWISTING_VINES || block == Blocks.TWISTING_VINES_PLANT;
    ...
    private static void installHook() {
        ...
        WorldRenderEvents.AFTER_ENTITIES.register(context -> {
            synchronized (INSTANCES) {
                for (ExtendedESPModule module : INSTANCES) {
    ...
    private boolean outOfRange(Entity entity, Vec3 camera) { return camera.distanceToSqr(entity.position()) > range * range; }

    private static void emitBox(PoseStack.Pose pose, VertexConsumer consumer, AABB box) {
        float minX = (float) box.minX, minY = (float) box.minY, minZ = (float) box.minZ, maxX = (float) box.maxX, maxY = (float) box.maxY, maxZ = (float) box.maxZ;
        line(pose, consumer, minX, minY, minZ, maxX, minY, minZ); line(pose, consumer, maxX, minY, minZ, maxX, minY, maxZ); line(pose, consumer, maxX, minY, maxZ, minX, minY, maxZ); line(pose, consumer, minX, minY, maxZ, minX, minY, minZ);
        line(pose, consumer, minX, maxY, minZ, maxX, maxY, minZ); line(pose, consumer, maxX, maxY, minZ, maxX, maxY, maxZ); line(pose, consumer, maxX, maxY, maxZ, minX, maxY, maxZ); line(pose, consumer, minX, maxY, maxZ, minX, maxY, minZ);
        line(pose, consumer, minX, minY, minZ, minX, maxY, minZ); line(pose, consumer, maxX, minY, minZ, maxX, maxY, minZ); line(pose, consumer, maxX, minY, maxZ, maxX, maxY, maxZ); line(pose, consumer, minX, minY, maxZ, minX, maxY, maxZ);
    }
