...
public final class OurClientClickGui extends Screen {
    ...
    @Override
    protected void init() {
        // No vanilla widgets are installed by this screen. Keeping focus empty avoids
        // the 1.21.11 Screen focus/navigation path that previously crashed on clicks.
        clearFocus();
        clampScroll();
    }

    private int sidebarWidth() {
        return Math.min(SIDEBAR_MAX, Math.max(SIDEBAR_MIN, width / 4));
    }

    private int contentX() {
        return 18 + sidebarWidth() + 18;
    }

    private int contentWidth() {
        return Math.max(120, width - contentX() - 18);
    }
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < CATEGORY_NAMES.length; i++) {
            ...
            g.fill(left + 8, y, left + sidebar - 8, y + 27,
                active ? SELECTED : (hover ? HOVER : ROW));
        ...
        g.drawString(font, "Mouse wheel: scroll   ↑/↓: select   Enter: toggle   Right-click: settings",
            contentX + 18, TOP + 30, MUTED, false);
        ...
        g.fill(searchLeft, searchY, searchLeft + searchWidth, searchY + 30,
            editingSearch || searchHover ? HOVER : ROW);
        String searchText = search.isEmpty() ? "Search modules..." : search;
        g.drawString(font, searchText + (editingSearch ? "_" : ""),
            searchLeft + 10, searchY + 10, search.isEmpty() ? MUTED : TEXT, false);
    ...
    private void renderModuleViewport(GuiGraphics g, int mouseX, int mouseY, List<ClientModule> modules) {
        ...
        int last = Math.min(modules.size() - 1,
            (int) Math.ceil((moduleScroll + viewport) / (ROW_HEIGHT + ROW_GAP)));
        ...
        for (int i = first; i <= last; i++) {
            ...
            int background = selected ? SELECTED : (hover ? HOVER : ROW);
            g.fill(x, y, x + w - 8, y + ROW_HEIGHT, background);
            g.fill(x, y, x + 3, y + ROW_HEIGHT, active ? ACCENT : 0x00000000);
            g.drawString(font, pretty(module.id()), x + 12, y + 7, TEXT, false);
            g.drawString(font, active ? "ON" : "OFF", x + w - 48, y + 7,
        ...
        if (maxScroll > 0) {
            ...
            int thumbHeight = Math.max(24,
                (int) ((double) viewport / Math.max(viewport, totalContentHeight(modules.size())) * trackHeight));
        ...
        if (moduleScroll > 0) {
            g.fill(x, top, x + w - 8, top + 3, 0xAA0B0B10);
        }
        if (moduleScroll < maxScroll) {
            g.fill(x, bottom - 3, x + w - 8, bottom, 0xAA0B0B10);
        }
    }
    ...
    private int maxScroll(int count) {
        return Math.max(0, totalContentHeight(count) - viewportHeight());
    }

    private void clampScroll() {
        clampScroll(modulesForCategory().size());
    }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
        try {
            ...
            if (inside(mx, my, sx, searchY(), sw, 30)) {
                editingSearch = true;
                settingsId = null;
                editingSetting = null;
                return true;
            ...
            if (inside(mx, my, listX, top, listW, bottom - top)) {
                ...
                if (within < ROW_HEIGHT && index >= 0 && index < modules.size()) {
                    ...
                    if (button == GLFW.GLFW_MOUSE_BUTTON_1 && modules.get(index) instanceof ToggleableModule t) {
                        toggle(modules.get(index).id(), t);
                    } else if (button == GLFW.GLFW_MOUSE_BUTTON_2) {
                        settingsId = modules.get(index).id();
                        editingSetting = null;
                    }
                    return true;
                }
    ...
    private boolean handleSettingsClick(double mx, double my) {
        ...
        if (inside(mx, my, x + w - 90, y, 90, 34)) {
            settingsId = null;
            editingSetting = null;
            return true;
        }
        int row = y + 48;
        if (module instanceof ToggleableModule t && inside(mx, my, x, row, w, 30)) {
            toggle(settingsId, t);
            return true;
        }
        row += 38;
    ...
    public boolean keyPressed(KeyEvent event) {
        try {
            ...
            if (settingsId != null) {
                if (editingSetting != null && (key == GLFW.GLFW_KEY_LEFT || key == GLFW.GLFW_KEY_RIGHT)) {
            ...
            if (key == GLFW.GLFW_KEY_PAGEUP) {
                moduleScroll -= viewportHeight() * .8;
                clampScroll(modules.size());
                return true;
            }
            if (key == GLFW.GLFW_KEY_PAGEDOWN) {
                moduleScroll += viewportHeight() * .8;
                clampScroll(modules.size());
                return true;
            }
            ...
            if (key == GLFW.GLFW_KEY_END && !modules.isEmpty()) {
