...
public final class OurClientClickGui extends Screen {
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float delta) {
        ...
        int contentX = left + sidebar + 18;
        int contentW = width - contentX - 18;
        ...
        g.drawString(font, searchText + (editingSearch ? "_" : ""), contentX + 24, searchY + 10,
                search.isEmpty() ? MUTED : TEXT, false);

        List<ClientModule> modules = modulesForCategory();
        if (modules.isEmpty()) {
            g.drawString(font, "No matching modules", contentX + 24, searchY + 55, MUTED, false);
        } else {
            ...
            for (int i = 0; i < modules.size(); i++) {
                ...
                g.fill(contentX + 14, y, contentX + contentW - 14, y + 30,
                        selected ? (active ? SELECTED : HOVER) : (active ? SELECTED : ROW));
                g.drawString(font, pretty(module.id()), contentX + 24, y + 10, TEXT, false);
                String state = active ? "ON" : "OFF";
                g.drawString(font, state, contentX + contentW - 58, y + 10, active ? TEXT : MUTED, false);
    ...
    private void toggle(String id, ToggleableModule t) {
        ...
        catch (RuntimeException e) { OurClient.LOGGER.error("Failed to toggle '{}'", id, e); try { t.setEnabled(false); } catch (RuntimeException ignored) {} }
    }
    ...
    public boolean charTyped(CharacterEvent event) {
        if (editingSearch && !Character.isISOControl(event.codepoint())) { search += new String(Character.toChars(event.codepoint())); selectedIndex = 0; return true; }
